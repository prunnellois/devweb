...
$('#primary-menu li .btn').removeClass('btn-outline-dark');
var navbarH = $('.my-navbar').height();
$(window).scroll(function() {
    if ($(window).scrollTop() > 1) {
        $('.my-navbar').addClass('fixed');
        $('#primary-menu li .btn').addClass('btn-outline-dark')
    } else {
        $('.my-navbar').removeClass('fixed');
        $('#primary-menu li .btn').removeClass('btn-outline-dark')
    }
...
$(window).resize(function() {
    if($(window).width() <= 768) {
        if (re === 0){
            $('#primary-menu li a .btn').contents().filter(function () {
                if (!$(this).is("i")) {
                    return this;
                }
            }).remove();
        ...
        if (re === 1){
            $('#primary-menu li a').contents().filter(function () {
                if ($(this).is("button")){
                    if (!$(this).is("i")) {
                        console.log($(this).text());
                        $(this).append(" "+this.parentElement.id+" ");
...
$('.div-foot').append("<div class=\"bubbles\">\n" +
